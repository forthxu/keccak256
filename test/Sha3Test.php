...
class Keccak256Test extends PHPUnit_Framework_TestCase
{
...
const long = "";
    ...
    public function  testKeccak256()
    {
        $v = [
            512 => [
                ...
                [hex2bin(Keccak256Test::short), '001618372e75147af90c0cf16c3bbdaa069ddbc62483b392d028ded49f75084a5dfcc53aecd9f57ddbb73daa041fd71089d8fb5edf6cfaf6f1e4e25ad3de266c'],
                [hex2bin(Keccak256Test::long), '6e8b8bd195bdd560689af2348bdc74ab7cd05ed8b9a57711e9be71e9726fda4591fee12205edacaf82ffbbaf16dff9e702a708862080166c2ff6ba379bc7ffc2']
            ],
            384 => [
                ...
                [hex2bin(Keccak256Test::short), 'feee2ef332515284e0ba247c62f264199044d03877c58e54b51a62e39e91c27aaae384837eb9d479b4c0308cfc6b779b'],
                [hex2bin(Keccak256Test::long), '128dc611762be9b135b3739484cfaadca7481d68514f3dfd6f5d78bb1863ae68130835cdc7061a7ed964b32f1db75ee1'],
            ...
            256 => [
                ...
                [hex2bin(Keccak256Test::short), '2c7e7cb356fdc68ec8927e499d2a6bae2b781817919c829ebbe8225baed46967'],
                [hex2bin(Keccak256Test::long), 'c11f3522a8fb7b3532d80b6d40023a92b489addad93bf5d64b23f35e9663521c'],
            ...
            224 => [
                ...
                [hex2bin(Keccak256Test::short), 'b1571bed52e54eef377d99df7be4bc6682c43387f2bf9acc92df608f'],
                [hex2bin(Keccak256Test::long), '94689ea9f347dda8dd798a858605868743c6bd03a6a65c6085d52bed'],
             ],
        ...
        foreach($v as $bitsize => $vectors){
            foreach($vectors as $testcase){
                $this->assertEquals(Keccak256::hash($testcase[0], $bitsize), $testcase[1]);

                $this->assertEquals(Keccak256::hash($testcase[0], $bitsize, true), hex2bin($testcase[1]));

                $class = new \ReflectionClass('\Keccak256\Keccak256');
    ...
    public function testShake()
    {
       $v = [
            128 => [
                ...
                [256, hex2bin(Keccak256Test::short), '3a0faca70c9d2b81d1064d429ea3b05ad27366f64985379ddd75bc73d6a83810'],
                [256, hex2bin(Keccak256Test::long), '14236e75b9784df4f57935f945356cbe383fe513ed30286f91060759bcb0ef4b'],
            ...
            256 => [
                ...
                [512, hex2bin(Keccak256Test::short), '57119c4507f975ad0e9ea4f1166e5f9b590bf2671aaeb41d130d2c570bafc579b0b9ec485cc736a0a848bbc886cbaa79ffcd067ce64b3b410741ab011c544225'],
                [512, hex2bin(Keccak256Test::long), '8a5199b4a7e133e264a86202720655894d48cff344a928cf8347f48379cef347dfc5bcffab99b27b1f89aa2735e23d30088ffa03b9edb02b9635470ab9f10389'],
            ]
        ];

        foreach($v as $bitsize => $vectors){
            foreach($vectors as $testcase){
                $this->assertEquals(Keccak256::shake($testcase[1], $bitsize, $testcase[0]), $testcase[2]);

                $this->assertEquals(Keccak256::shake($testcase[1], $bitsize, $testcase[0], true), hex2bin($testcase[2]));
    ...
    public function testSelfTestException()
    {
        ...
        try  {
            ...
            $this->assertEquals($e->getMessage(), 'Keccak256 self test failed!');
    ...
    public function testSelfTestPreviousFailure()
    {
        ...
        $this->setExpectedException('Exception', 'Keccak256 previous self test failed!');
        Keccak256::hash('',224);
    }
    ...
    {
        $this->setExpectedException('Exception', 'Unsupported Keccak256 Hash output size.');
        Keccak256::hash('',225);
    }

    public function testUnsupportedShakeSecurityLevel()
    {
        $this->setExpectedException('Exception', 'Unsupported Keccak256 Shake security level.');
        Keccak256::shake('',129, 256);
    }
}